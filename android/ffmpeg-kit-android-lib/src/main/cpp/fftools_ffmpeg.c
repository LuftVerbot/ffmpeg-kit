...
/* On Android, the binder threadpool is already initialized by the runtime
   before JNI code runs, so this is a no-op stub. */
#if CONFIG_MEDIACODEC
static void android_binder_threadpool_init_if_required(void) {}
#endif
...
__thread const char *program_name = "ffmpeg";
__thread int program_birth_year = 2000;
...
__thread atomic_uint nb_output_dumped = 0;
...
static __thread int received_sigterm = 0;
static __thread int received_nb_signals = 0;
static __thread atomic_int transcode_init_done = 0;
static __thread int ffmpeg_exited = 0;
static __thread int64_t copy_ts_first_pts = AV_NOPTS_VALUE;
                         ...
                         void *logctx, int decode)
{
    ...
    while ((e = av_dict_iterate(opts, e))) {
        ...
        option = av_opt_find(&clazz, optname, NULL, 0,
...
/** Report callback function pointer for ffmpeg-kit statistics */
static __thread void (*report_callback)(int, float, float, int64_t, double, double, double) = NULL;
...
void forward_report(int is_last_report, float fps, float quality, int64_t total_size, double pts, double bitrate, double speed) {
    if (report_callback != NULL) {
        report_callback(is_last_report, fps, quality, total_size, pts, bitrate, speed);
    }
}
...
int ffmpeg_execute(int argc, char **argv)
{
    ...
    show_help_default = ffmpeg_show_help_default;
